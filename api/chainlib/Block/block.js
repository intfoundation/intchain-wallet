...
class Block extends AbstractBlock {
    ...
    makeMerkleRoot() {
        let root = this.createMerkleRoot("hex")
        if (root) {
            this.merkleRoot = root
        } else {
            throw "bad-txns-duplicate"
        }
    }
    ...
    verifyBody() {
        const [valid,errinfo] = this.checkBody();
        if (!valid) {
            console.log('[block verifyBody], failed ,errinfo='+errinfo.toString());
        }
        return valid;
    }
    ...
    getJSON(network, view, height, confirmations) {
