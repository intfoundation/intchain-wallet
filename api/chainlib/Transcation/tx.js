...
class TX {
    ...
    setData(dataBuffer) {
        //only empty tx or dataTX can setData as a DataTX;
        if (this.inputs.length !== 0 && !this.inputs[0].isData()) {
            return false;
        }
        //clear prev data
        this.extData = dataBuffer;
        this._datapubkey = null;
        this._datasig = null;
        this.inputs = [];
        let infoInput = new Input({ isData: true });
        this.inputs.push(infoInput);
    ...
    signData(senderAccount) {
        if (!this.extData) {
            throw new Error('no data to sign');
        }
        this._datapubkey = senderAccount.getPublicKey();
        this._datasig = senderAccount.signHash(this.extData);

        this.inputs[0].script.pushData(this._datapubkey);
        this.inputs[0].script.pushData(this._datasig);
        this.inputs[0].script.compile();
    ...
    check(view, flags) {
        ...
        if (this.isCoinbase()) {
            //check coinbase signeature
            let sign = this.getCoinbaseFlag();
            let allOutputAddrs = '';
            for (let addr of this.getOutputAddresses()) {
                allOutputAddrs += addr.toString();
            }
            let systemKeyRing = KeyRing.fromPublic(Buffer.from(config.systemPubKey, 'hex'));
            if (!systemKeyRing.verifyHash(allOutputAddrs, sign)) {
                throw new ScriptError('UNKNOWN_ERROR', 'coinbase MUST create by system');
            }
            return;
        } else if (this.isDataTX()) {
            if (!this.extData) {
                throw new ScriptError('UNKNOWN_ERROR', 'MUST have extData');
            }
            if (!this._datapubkey || !this._datasig) {
        ...
        for (let i = 0; i < this.inputs.length; i++) {
            ...
            if (!view.isUnspent(prevout)) {
                throw new ScriptError('UNKNOWN_ERROR', 'coin already spend.');
            }
    ...
    isDataTX() {
        return this.inputs.length === 1 && this.inputs[0].isData();
    }
    ...
    checkSanity() {
        ...
        if (!this.isDataTX() && this.outputs.length === 0)
        ...
        for (const input of this.inputs) {
            if (input.isData() || input.isCoinbase()) {
                continue;
            }
        ...
        if (this.isCoinbase()) {
            ...
            const size = this.inputs[0].script.getSize();
            if (size < 2 || size > 100)
                return [false, 'bad-it-length', 100];
        } else {
            for (const input of this.inputs) {
    ...
    static createDataTX(data, senderAccount) {
        return new TX().setData(data).signData(senderAccount);
    }
    ...
    getNormalSizes() {
        ...
        if (this.extData) {
            base += encoding.sizeVarint(this.extData.length);
            base += this.extData.length;
        }
    ...
    getCoinbaseFlag() {
        assert(this.isCoinbase(), 'tx must be a coinbase!');
        return this.inputs[0].script.getCoinbaseFlag();
    }
}
/*
 * Helpers
